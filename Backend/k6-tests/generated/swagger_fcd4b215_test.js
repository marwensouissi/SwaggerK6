...
const af = post_abstract_with_payload({
  url: `https://dev-itona.xyz:443/api/edge/{edgeId}/asset/{assetId}`,
  token: token,
  tag: "af",
  job: "af",
    payload: null
});
console.log('🧪 af response:', JSON.stringify(af.data));
 
const ga = get_abstract_without_payload({
  url: `https://dev-itona.xyz:443/api/admin/autoCommitSettings`,
  token: token,
  tag: "ga",
  job: "ga"
});
console.log('🧪 ga response:', JSON.stringify(ga.data));
