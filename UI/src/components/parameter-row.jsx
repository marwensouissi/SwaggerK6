...
export default class ParameterRow extends Component {
  ...
  onChangeWrapper = (value, isXml = false) => {
    let { onChange, rawParam, oas3Actions, pathMethod, requestData } = this.props;
    ...
    const paramKey = `${rawParam.get("name")}-${rawParam.get("in")}`;
  
    if (Array.isArray(pathMethod) && pathMethod.length === 2) {
      const existingParams =
        requestData?.getIn([pathMethod[0], pathMethod[1], "params"])?.toJS() || {};
  
      const updatedParams = {
        ...existingParams,
        [paramKey]: valueForUpstream,
      };
  
      oas3Actions.setRequestFunctionParams({
        pathMethod,
