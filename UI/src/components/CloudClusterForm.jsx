...
const CloudClusterForm = ({ onBack, filename }) => { // <-- include filename
  ...
  const podIpMappings = JSON.parse(localStorage.getItem("podIpMappings") || "[]");
...
const filteredPodMappings = podIpMappings.filter(entry => {
  // Remove file extension from filename for comparison
  const baseFilename = filename ? filename.replace(/\.[^/.]+$/, "") : "";
  
  // Match if the folder starts with the base filename
  // e.g., "swagger" matches "swagger-2d5e0936"
  return entry.folder && entry.folder.startsWith(baseFilename);
});
...
if (message.startsWith("POD_NAME=")) {
 ...
 if (message.includes("SERVICE=")) {
    const serviceLine = message.replace("SERVICE=", "").trim();
    const [serviceName, ip] = serviceLine.split(":");
    const folderKey = serviceName.split("-test")[0];
    const podName = podNamesRef.current.find(pod => pod.startsWith(folderKey));

    if (podName && ip) {
      const existing = JSON.parse(localStorage.getItem("podIpMappings") || "[]");
      const isDuplicate = existing.some(e => e.folder === folderKey && e.pod === podName && e.ip === ip);
      if (!isDuplicate) {
        const updated = [...existing, { folder: folderKey, pod: podName, ip }];
        localStorage.setItem("podIpMappings", JSON.stringify(updated));
        console.log("📌 Saved pod→IP mapping:", { folder: folderKey, pod: podName, ip });
      }
    }
  }
  ...
  {cloudFolders.length > 0 ? (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {cloudFolders.map(folder => {
        ...
        const matchedService = services.find(service => service.startsWith(folder));

   // Filter pod mappings for THIS specific folder
const folderPodMappings = podIpMappings.filter(entry => {
  // Remove file extension from folder name for comparison
  const baseFolderName = folder ? folder.replace(/\.[^/.]+$/, "") : "";
  
  // Check if the current folder starts with the pod mapping folder
  // e.g., "swagger-c107efdd-test" starts with "swagger-c107efdd"
  return entry.folder && baseFolderName.startsWith(entry.folder);
});
        return (
...
<div className="flex flex-col gap-2">
            {folderPodMappings.length > 0 ? (
              folderPodMappings.map((entry, idx) => (
 <button
      key={idx}
      onClick={() => window.open(`http://${entry.ip}:5665`, "_blank")}
      title={`Open ${entry.ip} in browser`}
      style={{
        background: 'linear-gradient(145deg, #1a202c, #0d1117)',
        width: '40px',
        height: '40px',
    ...
    >
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M2 17V7C2 5.89543 2.89543 5 4 5H20C21.1046 5 22 5.89543 22 7V17C22 18.1046 21.1046 19 20 19H4C2.89543 19 2 18.1046 2 17Z" stroke="#ffffff" strokeWidth="2"/>
    ...
    <div className="text-gray-500 text-sm">
      No services found for folder: {filename}
    </div>
  )}
</div>
